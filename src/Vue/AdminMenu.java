...
public class AdminMenu {
			...
			for (int i = 0; i < res.size(); i++) {
				if (!res.get(i).isAccepted()) {
					System.out.println((cpt + 1) + res.get(i).toString());
					 ...
					 //for(int j=0; j<listReservations.getReservations().size(); j++)
						 //{System.out.println(listReservations.getReservations().get(i).toString());
						 //}
						// }
					// this.listReservations.reinitialize();
					cpt++;
	...
	private void removeUser() {
		...
		ArrayList<Student> studients = this.database().getStudents();
		...
		if (!found) {
			ArrayList<Teacher> teachers = this.database().getTeachers();
			for (int i = 0; i < this.database().getTeachers().size(); i++) {
	...
	private void displayUsers() {
		...
		ArrayList<Administrator> admins = this.database
				.getAdministrators();
...
private void statistiques(){
		...
		switch(rep){
		case 1:
			System.out.println("le material le plus emprunté est actuellement: "+this.listBorrowing.leplusEmprunte());
			break;
		case 2:
			this.emprunteur();
			break;
...
public void emprunteur(){
	...
	System.out.println("Actuellement le plus gros emprunteur est: "+ b.toString());
}
